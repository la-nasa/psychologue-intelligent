...
**Addendum ADR-005 (2026-08-28)** : le cœur conversationnel n'est plus non génératif par défaut. Sur retour direct de l'utilisateur (conversations perçues comme scriptées), un modèle génératif auto-hébergé (Qwen2.5-3B-Instruct, Apache-2.0) répond désormais aux messages GREEN, en écart explicite et documenté à ADR-003 — voir `docs/architecture/decision-records/ADR-005-generative-responder.md`. L'invariant d'ADR-004 (le LLM ne décide jamais d'une crise) reste structurellement inchangé et vérifié par test. Les mentions « non génératif » ci-dessous décrivent l'état antérieur à cette date, conservées pour l'historique plutôt que réécrites.
