...
| Réponses du répondeur génératif (ADR-005) | Modèle auto-hébergé (Qwen2.5-1.5B-Instruct, réduit depuis 3B pour la latence) en place, routage ORANGE/RED structurellement inatteignable vérifié par test ; aucune revue humaine du contenu réellement généré, aucun corpus de tests de jailbreak/prompt-injection (Section 41 du prompt maître) | Revue clinique d'un échantillon réel de réponses générées ; corpus de tests adversariaux versionné contre le modèle déployé, pas seulement contre la logique de routage |
| Latence du répondeur génératif | Mesurée en conditions réelles : 30 s à plus d'une minute par réponse (CPU partagé, voir ADR-005) — trop lent pour un usage à volume réel | Un modèle plus petit et/ou un hébergement dédié/GPU, avec une mesure propre de plusieurs échantillons avant tout pilote |
